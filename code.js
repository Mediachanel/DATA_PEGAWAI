...
// Hybrid sync (Spreadsheet = master, MySQL = read DB via Cloudflare Worker)
// Konfigurasi disimpan di Script Properties:
// - SYNC_WORKER_BASE: mis. https://<worker>.workers.dev
// - SYNC_KEY: secret untuk header X-SYNC-KEY
const HYBRID_SHEET = DATA_SHEET;
const HYBRID_REQUIRED_HEADERS = ['sid', 'sync_status', 'sync_error', 'synced_at'];
...
const USERNAME_HEADER_MAP = {
  nama_ukpd: ['nama_ukpd', 'nama ukpd', 'ukpd'],
  username: ['username', 'user', 'user_id'],
  password: ['password', 'pass'],
  hak_akses: ['hak_akses', 'hak akses', 'role', 'hak'],
...
// =====================
// Hybrid: schema + SID + sync triggers
// =====================
function setHybridSyncConfig(baseUrl, syncKey) {
  const props = PropertiesService.getScriptProperties();
  props.setProperty('SYNC_WORKER_BASE', String(baseUrl || '').trim());
  props.setProperty('SYNC_KEY', String(syncKey || '').trim());
  return json({ ok: true });
}

function getHybridSyncConfig_() {
  const props = PropertiesService.getScriptProperties();
  return {
    base: String(props.getProperty('SYNC_WORKER_BASE') || '').trim(),
    key: String(props.getProperty('SYNC_KEY') || '').trim(),
  };
}

const HYBRID_SHEETS_CONFIG = [
  { sheet: DATA_SHEET, table: 'pegawai', type: 'pegawai' },
  { sheet: USER_SHEET, table: 'username', type: 'username' },
  { sheet: MUTASI_SHEET, table: 'usulan_mutasi', type: 'mutasi' },
  { sheet: PEMUTUSAN_SHEET, table: 'usulan_pemutusan_jf', type: 'pemutusan' },
  { sheet: QNA_SHEET, table: 'qna', type: 'qna' },
  { sheet: BEZETTING_SHEET, table: 'bezetting', type: 'bezetting' },
];

function ensureSheetSchema_(sheetName) {
  ...
  const missing = HYBRID_REQUIRED_HEADERS.filter(h => !normalized.includes(h));
  if (!missing.length) return { ok: true, added: [] };

  const nextHeader = header.slice();
  missing.forEach((h) => nextHeader.push(h));
  sheet.getRange(1, 1, 1, nextHeader.length).setValues([nextHeader]);
  return { ok: true, added: missing };
}

function backfillSIDForSheet_(sheetName) {
  ensureSheetSchema_(sheetName);
  const sheet = getSheet_(sheetName);
  const lastRow = sheet.getLastRow();
  if (lastRow <= 1) return json({ ok: true, updated: 0 });

  const header = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0] || [];
  const keys = header.map(normalizeHeaderKey);
  const sidCol = keys.indexOf('sid') + 1;
  if (sidCol <= 0) throw new Error('Kolom SID tidak ditemukan setelah schema prep');

  const chunkSize = 2000;
  let updated = 0;
  for (let startRow = 2; startRow <= lastRow; startRow += chunkSize) {
    const count = Math.min(chunkSize, lastRow - startRow + 1);
    const range = sheet.getRange(startRow, sidCol, count, 1);
...
function ensurePegawaiSchema_() {
  const sheet = getSheet_(HYBRID_SHEET);
  if (!sheet) throw new Error(`Sheet ${HYBRID_SHEET} tidak ditemukan`);
  const lastCol = Math.max(1, sheet.getLastColumn());
  const header = sheet.getRange(1, 1, 1, lastCol).getValues()[0] || [];
  const normalized = header.map(normalizePegawaiHeader);
...
function ensurePegawaiSchema() {
  return ensurePegawaiSchema_();
}

function ensureAllHybridSchemas() {
  HYBRID_SHEETS_CONFIG.forEach((cfg) => ensureSheetSchema_(cfg.sheet));
  return json({ ok: true });
}

function backfillSID() {
  ensurePegawaiSchema_();
  const sheet = getSheet_(HYBRID_SHEET);
  const lastRow = sheet.getLastRow();
  if (lastRow <= 1) return json({ ok: true, updated: 0 });
...
function runBackfillSID() {
  return backfillSID();
}

function backfillAllSID() {
  HYBRID_SHEETS_CONFIG.forEach((cfg) => backfillSIDForSheet_(cfg.sheet));
  return json({ ok: true });
}

function auditNipDuplicates() {
  ensurePegawaiSchema_();
  const sheet = getSheet_(HYBRID_SHEET);
  const values = getSheetValues_(sheet);
  if (!values.length) return json({ ok: true, duplicates: [] });
  const [header, ...rows] = values;
  const keys = header.map(normalizePegawaiHeader);
  const nipIdx = keys.indexOf('nip');
  const sidIdx = keys.indexOf('sid');
  const nameIdx = keys.indexOf('nama_pegawai');
  if (nipIdx < 0) return json({ ok: false, error: 'Kolom nip tidak ditemukan' });

  const map = new Map();
  rows.forEach((r, i) => {
    const nip = String(r[nipIdx] || '').trim();
    if (!nip) return;
    const list = map.get(nip) || [];
    list.push({
      rowNumber: i + 2,
      sid: sidIdx >= 0 ? String(r[sidIdx] || '').trim() : '',
      nama: nameIdx >= 0 ? String(r[nameIdx] || '').trim() : '',
    });
    map.set(nip, list);
  });

  const duplicates = [];
  for (const [nip, list] of map.entries()) {
    if (list.length > 1) duplicates.push({ nip, rows: list });
  }
  duplicates.sort((a, b) => b.rows.length - a.rows.length);
  return json({ ok: true, duplicates: duplicates.slice(0, 200), total_duplicates: duplicates.length });
}

function touchUpdatedAt(rowNumber) {
  ensurePegawaiSchema_();
  const sheet = getSheet_(HYBRID_SHEET);
  const header = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0] || [];
  const keys = header.map(normalizePegawaiHeader);
  const updatedAtCol = keys.indexOf('updated_at') + 1;
  if (updatedAtCol <= 0) throw new Error('Kolom updated_at tidak ditemukan');
  sheet.getRange(rowNumber, updatedAtCol).setValue(new Date().toISOString());
}

function installTriggers() {
  const triggers = ScriptApp.getProjectTriggers();
  triggers.forEach((t) => {
    if (t.getHandlerFunction && (t.getHandlerFunction() === 'onEdit' || t.getHandlerFunction() === 'handleSyncOnEdit')) ScriptApp.deleteTrigger(t);
  });
  ScriptApp.newTrigger('handleSyncOnEdit').forSpreadsheet(SpreadsheetApp.openById(SPREADSHEET_ID)).onEdit().create();
  return json({ ok: true });
}

function handleSyncOnEdit(e) {
  try {
    if (!e || !e.range) return;
    const sheet = e.range.getSheet();
    if (!sheet) return;
    const sheetName = sheet.getName();
    const cfg = HYBRID_SHEETS_CONFIG.find((c) => c.sheet === sheetName);
    if (!cfg) return;
    ...
    for (let rowNumber = Math.max(2, rowStart); rowNumber <= rowEnd; rowNumber++) {
      if (cfg.type === 'pegawai') {
        ensurePegawaiSchema_();
        syncEditedRow_(sheet, rowNumber);
      } else {
        syncEditedRowGeneric_(sheetName, sheet, rowNumber, cfg);
      }
    }
  } catch (err) {
    console.error('Sync Error:', err); // Log error agar bisa dipantau di Executions
  }
}

function syncEditedRowGeneric_(sheetName, sheet, rowNumber, cfg) {
  const guardKey = `hybrid_edit:${sheet.getSheetId()}:${rowNumber}:${cfg.table}`;
  const cache = CacheService.getScriptCache();
  if (cache.get(guardKey)) return;
  ...
  const keys = header.map(normalizeHeaderKey);
  const sidIdx = keys.indexOf('sid');
  if (sidIdx < 0) throw new Error('Kolom sid tidak ditemukan');

  const row = sheet.getRange(rowNumber, 1, 1, keys.length).getValues()[0] || [];
  const sid = String(row[sidIdx] || '').trim() || Utilities.getUuid();

  const sidCol = sidIdx + 1;
  if (!String(row[sidIdx] || '').trim()) sheet.getRange(rowNumber, sidCol).setValue(sid);

  const updatedAtCol = keys.indexOf('updated_at') + 1;
  const updatedAt = new Date().toISOString();
  if (updatedAtCol > 0) sheet.getRange(rowNumber, updatedAtCol).setValue(updatedAt);

  const obj = buildRowObjectForSheet_(cfg.type, header, row);
  obj.sid = sid;
  if (updatedAtCol > 0 && !obj.updated_at) obj.updated_at = updatedAt;

  const payload = {
    sid,
    nip: obj.nip || null,
    row: obj,
    row_hash: computeRowHashGeneric_(obj),
  };

  const config = getHybridSyncConfig_();
  if (!config.base || !config.key) return;

  const url = String(config.base || '').replace(/\/+$/, '') + `/sync/${cfg.table}`;
  ...
  try {
    ...
    const errCol = keys.indexOf('sync_error') + 1;
    const statusCol = keys.indexOf('sync_status') + 1;
    if (statusCol > 0) sheet.getRange(rowNumber, statusCol).setValue('NET_ERR');
    if (errCol > 0) sheet.getRange(rowNumber, errCol).setValue(String(netErr.message).slice(0, 500));
    return;
  }

  const statusCol = keys.indexOf('sync_status') + 1;
  const errCol = keys.indexOf('sync_error') + 1;
  const syncedAtCol = keys.indexOf('synced_at') + 1;

  if (res.getResponseCode() >= 200 && res.getResponseCode() < 300) {
    if (statusCol > 0) sheet.getRange(rowNumber, statusCol).setValue('SYNCED');
    if (errCol > 0) sheet.getRange(rowNumber, errCol).setValue('');
    if (syncedAtCol > 0) sheet.getRange(rowNumber, syncedAtCol).setValue(new Date().toISOString());
  } else {
    const msg = `${res.getResponseCode()}: ${String(res.getContentText() || '').slice(0, 500)}`;
    if (statusCol > 0) sheet.getRange(rowNumber, statusCol).setValue('ERROR');
    if (errCol > 0) sheet.getRange(rowNumber, errCol).setValue(msg);
  }
}

function syncEditedRow_(sheet, rowNumber) {
  const guardKey = `hybrid_edit:${sheet.getSheetId()}:${rowNumber}`;
  const cache = CacheService.getScriptCache();
  if (cache.get(guardKey)) return;
  cache.put(guardKey, '1', 10);
  ...
  // anti-loop: set guard sebelum write
  const sidCol = sidIdx + 1;
  if (!String(row[sidIdx] || '').trim()) sheet.getRange(rowNumber, sidCol).setValue(sid);
  ...
  // validasi minimal
  const nama = String(nameIdx >= 0 ? (row[nameIdx] || '') : '').trim();
  if (!nama) return;

  // enforce unik NIP jika ada
  const nip = String(nipIdx >= 0 ? (row[nipIdx] || '') : '').trim();
  if (nip) {
    const values = getSheetValues_(sheet);
    const rows = values.slice(1);
    const duplicate = rows.some((r, i) => {
      if (i + 2 === rowNumber) return false;
      const rNip = String(nipIdx >= 0 ? (r[nipIdx] || '') : '').trim();
      return rNip && rNip === nip;
    });
    if (duplicate) throw new Error(`NIP duplikat: ${nip}`);
  }
  ...
  const payload = {
    sid,
    nip: nip || null,
    row: obj, // kirim seluruh row (header->value) agar tabel DB bisa mirror spreadsheet
    row_hash: computePegawaiRowHash_({
      sid,
      nip: nip || '',
      nama: obj.nama_pegawai || '',
      ukpd: obj.nama_ukpd || '',
      status_pegawai: obj.nama_status_aktif || '',
      jabatan: obj.nama_jabatan_orb || '',
      pendidikan: obj.jenjang_pendidikan || '',
  ...
  const url = String(config.base || '').replace(/\/+$/, '') + '/sync/pegawai';
...
function computePegawaiRowHash_(data) {
  const norm1 = (v) => String(v || '').trim().replace(/\s+/g, ' ');
  const up = (v) => norm1(v).toUpperCase();
  const input = [
    norm1(data.sid),
    norm1(data.nip),
    norm1(data.nama),
    up(data.ukpd),
    up(data.status_pegawai),
    norm1(data.jabatan),
    norm1(data.pendidikan),
    norm1(data.updated_at),
  ].join('|');
  const bytes = Utilities.computeDigest(Utilities.DigestAlgorithm.SHA_256, input, Utilities.Charset.UTF_8);
  return bytes.map(b => (b < 0 ? b + 256 : b).toString(16).padStart(2, '0')).join('');
}

function computeRowHashGeneric_(obj) {
  const keys = Object.keys(obj || {}).sort();
  const payload = keys.map(k => `${k}:${String(obj[k] ?? '')}`).join('|');
  const bytes = Utilities.computeDigest(Utilities.DigestAlgorithm.SHA_256, payload, Utilities.Charset.UTF_8);
  return bytes.map(b => (b < 0 ? b + 256 : b).toString(16).padStart(2, '0')).join('');
}

function fullRefreshToMySQL() {
  ensurePegawaiSchema_();
  const config = getHybridSyncConfig_();
  if (!config.base || !config.key) return json({ ok: false, error: 'SYNC_WORKER_BASE / SYNC_KEY belum diset (pakai setHybridSyncConfig)' });

  const sheet = getSheet_(HYBRID_SHEET);
  const values = getSheetValues_(sheet);
  if (!values.length) return json({ ok: true, sent: 0 });
  const [header, ...rows] = values;
  const keys = header.map(normalizePegawaiHeader);
  ...
  // pastikan semua SID ada (tanpa overwrite)
  backfillSID();
  const refreshedValues = getSheetValues_(sheet);
  const refreshedRows = refreshedValues.slice(1);

  const normalizeRow = (row) => {
    const obj = rowToObject(keys, row);
    const sid = String(obj.sid || '').trim();
    const nip = String(obj.nip || '').trim();
    const updatedAt = obj.updated_at ? String(obj.updated_at) : '';
    const payload = { sid, nip: nip || null, row: obj };
    payload.row_hash = computePegawaiRowHash_({
      sid,
      nip: nip || '',
      nama: obj.nama_pegawai || '',
      ukpd: obj.nama_ukpd || '',
  ...
  const postJson = (path, body) => UrlFetchApp.fetch(`${base}${path}`, {
    method: 'post',
    contentType: 'application/json',
  ...
  const startRes = postJson('/refresh/pegawai/start', {});
  if (startRes.getResponseCode() < 200 || startRes.getResponseCode() >= 300) {
    return json({ ok: false, error: `start failed: ${startRes.getResponseCode()} ${startRes.getContentText()}` });
  }
  const startBody = JSON.parse(startRes.getContentText() || '{}');
  const refreshId = startBody.refresh_id;
  if (!refreshId) return json({ ok: false, error: 'refresh_id kosong' });

  const chunkSize = 50;
  const rowsToSend = refreshedRows
    .filter(r => String(nameIdx >= 0 ? (r[nameIdx] || '') : '').trim() || String(nipIdx >= 0 ? (r[nipIdx] || '') : '').trim() || String(sidIdx >= 0 ? (r[sidIdx] || '') : '').trim())
    .map(normalizeRow);
  ...
  try {
    for (let i = 0; i < total; i += chunkSize) {
      ...
      const rowsChunk = rowsToSend.slice(i, i + chunkSize);
      const body = { refresh_id: refreshId, chunk_index: chunkIndex, rows: rowsChunk };

      let attempt = 0;
      while (true) {
        const res = postJson('/refresh/pegawai/chunk', body);
        const ok = res.getResponseCode() >= 200 && res.getResponseCode() < 300;
        if (ok) break;
        attempt++;
        if (attempt >= 5) throw new Error(`chunk ${chunkIndex} gagal: ${res.getResponseCode()} ${res.getContentText()}`);
        Utilities.sleep(Math.min(30000, 500 * Math.pow(2, attempt)));
      }
    }

    const commitRes = postJson('/refresh/pegawai/commit', { refresh_id: refreshId, expected_row_count: total });
    if (commitRes.getResponseCode() < 200 || commitRes.getResponseCode() >= 300) {
      throw new Error(`commit gagal: ${commitRes.getResponseCode()} ${commitRes.getContentText()}`);
    }
  } catch (err) {
    const msg = String(err && err.message ? err.message : err).slice(0, 500);
    if (statusCol > 0 || errCol > 0) {
      const lastRow = sheet.getLastRow();
      const batch = 2000;
      for (let startRow = 2; startRow <= lastRow; startRow += batch) {
        const count = Math.min(batch, lastRow - startRow + 1);
        if (statusCol > 0) sheet.getRange(startRow, statusCol, count, 1).setValues(Array(count).fill(['ERROR']));
        if (errCol > 0) sheet.getRange(startRow, errCol, count, 1).setValues(Array(count).fill([msg]));
      }
    }
    return json({ ok: false, refresh_id: refreshId, error: msg });
  }

  // mark all rows synced (batch)
  if (statusCol > 0 || errCol > 0 || syncedAtCol > 0) {
    const nowIso = new Date().toISOString();
    const lastRow = sheet.getLastRow();
    const batch = 2000;
    for (let startRow = 2; startRow <= lastRow; startRow += batch) {
      const count = Math.min(batch, lastRow - startRow + 1);
      if (statusCol > 0) sheet.getRange(startRow, statusCol, count, 1).setValues(Array(count).fill(['SYNCED']));
      if (errCol > 0) sheet.getRange(startRow, errCol, count, 1).setValues(Array(count).fill(['']));
      if (syncedAtCol > 0) sheet.getRange(startRow, syncedAtCol, count, 1).setValues(Array(count).fill([nowIso]));
    }
  }

  return json({ ok: true, refresh_id: refreshId, expected_row_count: total });
}

function fullRefreshSheetToMySQL_(cfg) {
  ensureSheetSchema_(cfg.sheet);
  const config = getHybridSyncConfig_();
  if (!config.base || !config.key) return json({ ok: false, error: 'SYNC_WORKER_BASE / SYNC_KEY belum diset (pakai setHybridSyncConfig)' });
  ...
  backfillSIDForSheet_(cfg.sheet);
  const refreshedValues = getSheetValues_(sheet);
  const refreshedRows = refreshedValues.slice(1);

  const normalizeRow = (row) => {
    const obj = buildRowObjectForSheet_(cfg.type, header, row);
    const sid = String(obj.sid || row[sidIdx] || '').trim();
    obj.sid = sid;
    if (!obj.updated_at && keys.includes('updated_at')) obj.updated_at = new Date().toISOString();
    const payload = { sid, nip: obj.nip || null, row: obj };
    payload.row_hash = computeRowHashGeneric_(obj);
    return payload;
  };
  ...
  const startRes = postJson(`/refresh/${cfg.table}/start`, {});
  if (startRes.getResponseCode() < 200 || startRes.getResponseCode() >= 300) {
    return json({ ok: false, error: `start failed: ${startRes.getResponseCode()} ${startRes.getContentText()}` });
  }
  const startBody = JSON.parse(startRes.getContentText() || '{}');
  ...
  try {
    for (let i = 0; i < total; i += chunkSize) {
      ...
      let attempt = 0;
      while (true) {
        const res = postJson(`/refresh/${cfg.table}/chunk`, body);
        const ok = res.getResponseCode() >= 200 && res.getResponseCode() < 300;
        if (ok) break;
        attempt++;
        if (attempt >= 5) throw new Error(`chunk ${chunkIndex} gagal: ${res.getResponseCode()} ${res.getContentText()}`);
    ...
    const commitRes = postJson(`/refresh/${cfg.table}/commit`, { refresh_id: refreshId, expected_row_count: total });
    if (commitRes.getResponseCode() < 200 || commitRes.getResponseCode() >= 300) {
      throw new Error(`commit gagal: ${commitRes.getResponseCode()} ${commitRes.getContentText()}`);
    }
  } catch (err) {
...
function fullRefreshAllToMySQL() {
  const results = [];
  HYBRID_SHEETS_CONFIG.forEach((cfg) => {
    const res = fullRefreshSheetToMySQL_(cfg);
    const raw = typeof res.getContent === 'function'
      ? res.getContent()
      : (typeof res.getContentText === 'function' ? res.getContentText() : String(res));
    let parsed = null;
    try { parsed = JSON.parse(raw); } catch (_) { parsed = { ok: false, raw }; }
    results.push({ sheet: cfg.sheet, table: cfg.table, result: parsed });
  });
  return json({ ok: true, results });
}

// =====================
// Hybrid: resumable refresh (batched to avoid timeout)
// =====================
function getRefreshState_() {
  const props = PropertiesService.getScriptProperties();
  const raw = String(props.getProperty('HYBRID_REFRESH_STATE') || '').trim();
  if (!raw) return {};
...
function setRefreshState_(state) {
  const props = PropertiesService.getScriptProperties();
  props.setProperty('HYBRID_REFRESH_STATE', JSON.stringify(state || {}));
}

function clearRefreshState_(table) {
...
function resetRefreshState(table) {
  clearRefreshState_(table);
  return json({ ok: true, cleared: table || 'all' });
}

function refreshTableBatch_(cfg, maxChunksPerRun) {
  ensureSheetSchema_(cfg.sheet);
  const config = getHybridSyncConfig_();
  if (!config.base || !config.key) {
    return json({ ok: false, error: 'SYNC_WORKER_BASE / SYNC_KEY belum diset (pakai setHybridSyncConfig)' });
  }

  const sheet = getSheet_(cfg.sheet);
  const values = getSheetValues_(sheet);
  if (!values.length) return json({ ok: true, sent: 0, done: true });
  const [header, ...rows] = values;
  const keys = header.map(normalizeHeaderKey);
  const sidIdx = keys.indexOf('sid');
  if (sidIdx < 0) throw new Error('Kolom sid tidak ditemukan');
  ...
  const rowsToSend = refreshedRows
    ...
    .map(normalizeRow);
  const total = rowsToSend.length;
  if (!total) return json({ ok: true, sent: 0, done: true });
  ...
  const state = getRefreshState_();
  const st = state[cfg.table] || {};

  let refreshId = st.refresh_id;
  let nextIndex = Number.isFinite(Number(st.next_index)) ? Number(st.next_index) : 0;
  const shouldRestart = !refreshId || st.total !== total;
  if (shouldRestart) {
    const startRes = postJson(`/refresh/${cfg.table}/start`, {});
    if (startRes.getResponseCode() < 200 || startRes.getResponseCode() >= 300) {
      return json({ ok: false, error: `start failed: ${startRes.getResponseCode()} ${startRes.getContentText()}` });
    ...
    const startBody = JSON.parse(startRes.getContentText() || '{}');
    refreshId = startBody.refresh_id;
    if (!refreshId) return json({ ok: false, error: 'refresh_id kosong' });
    nextIndex = 0;
  }

  const maxChunks = Math.max(1, Math.min(200, Number(maxChunksPerRun) || 20));
  let sent = 0;
  try {
    for (let i = nextIndex; i < totalChunks && sent < maxChunks; i++) {
      const start = i * chunkSize;
      const rowsChunk = rowsToSend.slice(start, start + chunkSize);
      const body = { refresh_id: refreshId, chunk_index: i, rows: rowsChunk };
      ...
      while (true) {
        ...
        attempt++;
        if (attempt >= 5) {
          throw new Error(`chunk ${i} gagal: ${res.getResponseCode()} ${res.getContentText()}`);
        }
        Utilities.sleep(Math.min(30000, 500 * Math.pow(2, attempt)));
      }
      sent++;
    ...
    setRefreshState_(state);
    return json({ ok: false, refresh_id: refreshId, error: state[cfg.table].error });
  }

  if (nextIndex >= totalChunks) {
    const commitRes = postJson(`/refresh/${cfg.table}/commit`, { refresh_id: refreshId, expected_row_count: total });
    if (commitRes.getResponseCode() < 200 || commitRes.getResponseCode() >= 300) {
      const msg = `commit gagal: ${commitRes.getResponseCode()} ${commitRes.getContentText()}`;
      state[cfg.table] = {
        refresh_id: refreshId,
        next_index: nextIndex,
        total,
    ...
    delete state[cfg.table];
    setRefreshState_(state);
    return json({ ok: true, refresh_id: refreshId, sent, total, done: true });
  }
  ...
  setRefreshState_(state);
  return json({ ok: true, refresh_id: refreshId, sent, total, done: false, next_index: nextIndex });
}

function fullRefreshAllToMySQLBatched(maxChunksPerRun) {
  const results = [];
  HYBRID_SHEETS_CONFIG.forEach((cfg) => {
    const res = refreshTableBatch_(cfg, maxChunksPerRun);
    const raw = typeof res.getContent === 'function'
      ? res.getContent()
      : (typeof res.getContentText === 'function' ? res.getContentText() : String(res));
    let parsed = null;
...
function fullRefreshTableBatched(table, maxChunksPerRun) {
  const name = String(table || '').trim();
  const cfg = HYBRID_SHEETS_CONFIG.find((c) => c.table === name || c.sheet === name);
  if (!cfg) return json({ ok: false, error: `table tidak ditemukan: ${name}` });
  return refreshTableBatch_(cfg, maxChunksPerRun);
}

// =====================
// Trigger helper: schedule full refresh daily
// =====================
function installDailyFullRefreshTrigger(hourLocal) {
  const hour = Number.isFinite(Number(hourLocal)) ? Number(hourLocal) : 2;
  installDailyTrigger_('fullRefreshToMySQL', hour);
  return json({ ok: true, trigger: 'fullRefreshToMySQL', hour });
}

function installDailyFullRefreshAllTrigger(hourLocal) {
  const hour = Number.isFinite(Number(hourLocal)) ? Number(hourLocal) : 2;
  installDailyTrigger_('fullRefreshAllToMySQL', hour);
  return json({ ok: true, trigger: 'fullRefreshAllToMySQL', hour });
}

function installDailyTrigger_(handlerName, hour) {
  const triggers = ScriptApp.getProjectTriggers();
  triggers.forEach((t) => {
  ...
  ScriptApp.newTrigger(handlerName)
    .timeBased()
    .atHour(Math.min(23, Math.max(0, hour)))
    .everyDays(1)
    .create();
}
...
function createPegawai(e) {
  ...
  return withLock_(() => {
    ...
    if (nipVal) {
      ...
      if (idxNip >= 0) {
        const existsNip = rows.some(r => String(r[idxNip] || '').trim() === nipVal);
        if (existsNip) return json({ ok: false, error: `NIP sudah ada: ${nipVal}` });
      }
    }
    ...
    const sidIdx = keys.indexOf('sid');
    if (sidIdx >= 0 && !body.sid) body.sid = Utilities.getUuid();

    const row = keys.map((k) => {
      if (!k) return '';
      if (k === 'sid') return body.sid || '';
      if (k === 'id') return body.id || body.sid || '';
    ...
    return json({ ok: true, data: { id: body.sid || body.id || body.nip || fallbackId || '' } });
  });
}
...
function updatePegawai(e) {
  ...
  return withLock_(() => {
    ...
    if (nextNip) {
      const idxNip = keys.indexOf('nip');
      if (idxNip >= 0) {
        const duplicate = rows.some((r, i) => {
          if (i === idx) return false;
          const rNip = String(r[idxNip] || '').trim();
          return rNip && rNip === nextNip;
        });
        if (duplicate) return json({ ok: false, error: `NIP sudah dipakai pegawai lain: ${nextNip}` });
      }
    }
...
function toUsernameRecord(header, row) {
  const headerIndex = buildHeaderIndex(header);
  const getVal = (key, fallbackIdx) => {
    const names = USERNAME_HEADER_MAP[key] || [key];
    const idx = findHeaderIndex(headerIndex, names);
    if (idx >= 0 && row[idx] !== undefined) return row[idx] || '';
    if (typeof fallbackIdx === 'number' && row[fallbackIdx] !== undefined) return row[fallbackIdx] || '';
    return '';
  ...
  return {
    nama_ukpd: getVal('nama_ukpd', 0),
    username: getVal('username', 1),
    password: getVal('password', 2),
    hak_akses: getVal('hak_akses', 3),
    wilayah: getVal('wilayah', 4),
  };
}

function buildRowObjectForSheet_(type, header, row) {
  if (type === 'pegawai') {
    const keys = (header || []).map(normalizePegawaiHeader);
    return rowToObject(keys, row);
  }
  if (type === 'username') return toUsernameRecord(header, row);
  if (type === 'mutasi') return toMutasiRecord(header, row);
  if (type === 'pemutusan') return toPemutusanRecord(header, row);
  if (type === 'qna') return toQnaRecord(header, row);
  if (type === 'bezetting') return toBezettingRecord(header, row);
  return {};
}
...
function getIdParam(e) {
  const body = e.body || parseBody(e) || {};
  return String(e?.parameter?.id || body.id || body.sid || body.nip || '').trim();
}
...
function toRecord(header, row, keys) {
  ...
  const idVal = obj.sid || obj.id || obj.nip || buildCompositeId(obj.nama_pegawai, obj.tanggal_lahir) || '';
...
function findRowIndexById(header, rows, id) {
  ...
  return rows.findIndex(r => {
    const sidVal = (idxSid >= 0 ? r[idxSid] : '')?.toString?.() || '';
    const idVal = (idxId >= 0 ? r[idxId] : '')?.toString?.() || '';
    const nipVal = (idxNip >= 0 ? r[idxNip] : '')?.toString?.() || '';
    return (sidVal && sidVal === id) || (idVal && idVal === id) || (nipVal && nipVal === id);
  });
}
...
function countStatus(rows) {
  return rows.reduce((acc, r) => {
    const raw = String(r.nama_status_aktif || '').trim();
    const k = raw ? raw.toUpperCase() : 'LAINNYA';
    acc[k] = (acc[k] || 0) + 1;
    return acc;
  }, {});
}
